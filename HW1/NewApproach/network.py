...
class Network:
    ...
    def backprop(self, x, y):
        ...
        for b, w in zip(self.biases, self.weights):
            z = [weight*activation for weight in w[:,0]] + b
            zs.append(z)
            activation = sigmoid(z)
            activations.append(activation)
       
        # backward pass
        delta = self.binary_cross_entropy(activations[-1], y) * sigmoid_prime(zs[-1])
        self.__error.append(delta) #save error for visualazation
        nabla_b[-1] = delta
        print(delta)
        ...
        for _layer in range(2, self.num_layers):
            z = zs[-_layer]
            delta = self.binary_cross_entropy(activations[-_layer-1], y) * sigmoid_prime(z)
